...
hosted_agent_is_model_throttle_error() {
  ...
  if grep -qihE \
      "^HTTP[/0-9.]*[[:space:]]+429([[:space:]]|$)|Error calling model:.*(Error code:[[:space:]]*429|rate_limit_exceeded|TooManyRequests|Model deployment rate limit exceeded)|['\"]code['\"][[:space:]]*:[[:space:]]*['\"](rate_limit_exceeded|TooManyRequests)['\"]|['\"]status_code['\"][[:space:]]*:[[:space:]]*429" \
      "$@" 2>/dev/null; then
    return 0
  fi
