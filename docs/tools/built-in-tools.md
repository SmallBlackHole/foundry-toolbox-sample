...
`CustomKeys` connection that carries the Bing key plus the account `ResourceId`.

Create the connection first (both ways). Use `--kind custom-keys`; the Bing key is sent as the
